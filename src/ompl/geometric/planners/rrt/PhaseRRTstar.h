/*
Copied skeleton PhaseRRTstar.h
author: @shuaiyy
*/

#ifndef OMPL_GEOMETRIC_PLANNERS_RRT_PHASERRTSTAR_
...
namespace ompl
{
    ...
    {
        /**
           @anchor gPhaseRRTstar
           @par Short description
           PhaseRRTstar is \ref gRRTstar "RRT*" over states (q, alpha), where
           alpha in [0,1] indexes a demonstration. Sampling and the per-edge
           phase-advance bound are those of \ref gPhaseRRT "PhaseRRT", so
           ordering along the demonstration holds by construction. The metric
           stays symmetric, and the direction is enforced instead by two phase
           windows: only earlier neighbours may parent a new motion, only later
           ones may be rewired through it. Rewiring changes a route and its
           cost, never a phase. Optimality is with respect to the
           OptimizationObjective in the ProblemDefinition, typically
           PhaseSimilarityObjective.
        ...
        */
        ...
        /** \brief Phase-monotone optimal Rapidly-exploring Random Trees */
        class PhaseRRTstar : public base::Planner
        {
        public:
            PhaseRRTstar(const base::SpaceInformationPtr &si);

            ~PhaseRRTstar() override;
            ...
            /** \brief Maximum phase advance per edge (forbids skipping ahead) */
            ...
            /** \brief Flat-alpha mode for constrained planning: alpha lives in
                ...
                stored as alpha * scale.  Pass the same weight the compound
            ...
            /** \brief Project phase-door samples onto a constraint manifold, so
                extensions aim at on-manifold targets. The constraint must leave
